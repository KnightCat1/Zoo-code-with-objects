...
int main()
{
	...
	z1.AddAnimal(new Horse("GRAY", 2, 20, 11, 3, "Selle Francais"));
	z1.AddAnimal(new Mermaid("BROWN", 3, 65.5, 9, 2, 2, 4, "Ariel", "Princess"));
	z1.AddAnimal(new GoldFish("GOLD", 5, 0.2, 0.01, 2, 3, 4, 0.321, 0.3231));
	...
	GoldFish* g = dynamic_cast<GoldFish*>(z4.GetAnimals()[2]);
	if (strcmp(g->GetColor(), "GOLD") != 0)
		cout << "Zoo3 fourth animal color is wrong after load (-1)" << endl;

	if (g->GetChildCount() != 5)
		cout << "Zoo3 fourth animal childs is wrong after load (-1)" << endl;

	if (g->GetLifetime() != 0.2)
		cout << "Zoo3 fourth animal lifetime is wrong after load (-1)" << endl;

	if (g->GetPregnanceTime() != 0.01)
		cout << "Zoo3 fourth animal pregnance is wrong after load (-1)" << endl;

	if (g->GetMilk() != 2)
		cout << "Zoo3 fourth animal milk is wrong after load (-1)" << endl;

	if (g->GetFinCount() != 3)
		cout << "Zoo3 fourth animal fins is wrong after load (-1)" << endl;

	if (g->GetGillsCount() != 4)
		cout << "Zoo3 fourth animal gills is wrong after load (-1)" << endl;

	if (g->GetWeight() != 0.321)
		cout << "Zoo3 fourth animal weight is wrong after load (-1)" << endl;

	if (g->GetLength() != 0.3231)
		cout << "Zoo3 fourth animal length is wrong after load (-1)" << endl;
	//finished
	cout << "done" << endl;
